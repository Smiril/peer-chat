...
if [ "$#" -ne 1 ]; then
    echo "USAGE: ${0} <cuda-9.1/cuda-9.2>"
    exit 0
fi

sudo zypper install patterns-openSUSE-base patterns-openSUSE-devel_basis patterns-openSUSE-devel_C_C++ git

if [ ! -f /usr/local/${1}/bin/nvcc ]; then
echo "Install CUDA Nvidia devel and toolkit packages!"
exit 0
else
echo "
#include <stdio.h>
...
void start(unsigned int *, int, const char *);
...
const char* Versionx() {
#ifdef DECENT_BOT_ARCH
#ifdef VERSION
  return VERSION\" - \"DECENT_BOT_ARCH;
#else
  return \"SHA1-BF 1.2 T.E.D. - The Enemy Dail - NICOLA\";
#endif
#else
  return \"SHA1-BF 1.2 T.E.D. - The Enemy Dail - NICOLA\";
#endif
...
int main()
{
    ...
    printf(\"Checking for every possible 1-999 character password. \\n\\n\");
...
void start(word * hash_tmp,  int length, const char * res)
{
    ...
    cudaMemcpy (hash, hash_tmp,sizeof(hash_tmp),cudaMemcpyHostToDevice);
...
/usr/local/${1}/bin/nvcc sha_function.cu -x cu -DDECENT_BOT_ARCH="\"TESLA\"" -DVERSION="\"SHA1-BF 1.3 T.E.D. - The Enemy Dail\"" -o sha1-bf -I/usr/local/${1}/include


sudo mv ./sha1-bf /bin/sha1-bf
